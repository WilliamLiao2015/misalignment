...
# epsilon_progress = 0
threshold_acceleration = constants.g * 0.6 / (10 ** 2) # 0.5g / (FPS^2)
epsilon_acceleration = 2e-2
epsilon_velocity = 2e-2
...
def is_accelerating(vector_map: VectorMap, state: StateArray) -> Optional[Tuple[int, int]]:
    for t in range(len(state) - minimum_period):
        ...
        if not np.all(diffs - threshold_acceleration > -epsilon_acceleration): continue
