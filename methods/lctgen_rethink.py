...
class OpenAIModel(BasicLLM):
    ...
    def llm_query(self, extended_prompt):
        if self.codex_cfg.MODEL == "debug":
        ...
        elif self.model.startswith("gpt-") or self.model.startswith("gemma-3-"):
            ...
            responses = self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": "Please rethink the scenario based on previous response."}
                ],
                temperature=self.codex_cfg.TEMPERATURE,
                max_tokens=self.codex_cfg.MAX_TOKENS,
                top_p = 1.,
...
def gen_scenario_from_gpt_text(llm_text, cfg, model, map_vecs, map_ids, save_image=False):
    ...
    if save_image:
        ...
        with open(os.path.join(os.path.dirname(__file__), "..", "data/lctgen_rethink-scenario.png"), "wb") as fp:
            image.save(fp)
